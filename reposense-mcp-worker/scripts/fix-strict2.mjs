...
/* cache.ts: ttl is always number, no implicit anys, KV cursor typing */
write(
  "src/cache.ts",
  `
  ...
  return {
    ...
    async clearAll(): Promise<{ deleted: number; warning?: string }> {
      ...
      for (;;) {
        ...
        const res: KVNamespaceListResult = await kv.list(opts);
...
/* github/auth.ts: preserve literal union types, no status widened to string */
write(
  "src/github/auth.ts",
  `
...
/* github/client.ts: omit rate property when undefined (exactOptionalPropertyTypes) */
write(
...
/* github/token_store.ts: force ArrayBuffer (not SharedArrayBuffer union) by copying */
write(
...
/* session.ts: force ArrayBuffer by copying msg into a new Uint8Array */
patch("src/session.ts", (s) => {
  if (s.includes('const sig = await crypto.subtle.sign("HMAC", key, ab);') && s.includes("new Uint8Array(msg).buffer")) return s;

  return s.replace(
    /const ab = msg\.buffer\.slice\(msg\.byteOffset, msg\.byteOffset \+ msg\.byteLength\);\s*\n\s*const sig = await crypto\.subtle\.sign\("HMAC", key, ab\);/m,
