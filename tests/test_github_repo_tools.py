...
async def test_repo_tree_branch_ref_resolves_to_sha(mcp_client, monkeypatch, tmp_path):
    ...
    # branch -> sha
    respx.get(
        "https://api.github.com/repos/vityk-dev/3DPrintingPortfolio/git/refs/heads/main"
    ).mock(return_value=httpx.Response(200, json={"object": {"sha": "abc123"}}))

    # sha -> tree
    respx.get("https://api.github.com/repos/vityk-dev/3DPrintingPortfolio/git/trees/abc123").mock(
        return_value=httpx.Response(
            200, json={"sha": "abc123", "tree": [{"path": "README.md", "type": "blob"}]}
        )
    )

    r = await mcp_client.call_tool(
        "github_repo_tree", {"owner": "vityk-dev", "repo": "3DPrintingPortfolio", "ref": "main"}
    )
